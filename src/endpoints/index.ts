...
export default class EndPoints {
    ...
    static getMusics = async ({ limit, offset, search }: Search) => {
        const { data } = await api.get(
            `/musics?limit=${limit}&offset=${offset}&search=${search}`,
        );
    ...
    static addMusicsToPlaylist = async (
        musics_ids: number[],
        playlist_id: number,
    ) => {
    ...
    static deleteMusicToPlaylist = async (
        playlist_id: string,
        music_id: number,
    ) => {
        const { data } = await api.delete(
            `/playlists/delete-music?playlist_id=${playlist_id}&music_id=${music_id}`,
        );
    ...
    static getLikedMusics = async () => {
        try {
            const response = await api.get("/musics/liked", {
                withCredentials: true,
            });
