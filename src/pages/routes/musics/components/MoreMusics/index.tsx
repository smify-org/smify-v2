...
export default function MoreMusics({
    ...
    const [favoritesMusic, setFavoritesMusic] = useState<{ favorite_music_id: number }[]>([]);

    const setLikeOrDeslike = async (music_id: number) => {
        await EndPoints.likeOrDeslike(music_id)

        const verifyFavoriteMusic = favoritesMusic.some(favorite => favorite.favorite_music_id === music_id)

        if (verifyFavoriteMusic) {
            setFavoritesMusic(favoritesMusic.filter(item => item.favorite_music_id !== music_id))
            return;
        }

        setFavoritesMusic(prev => [...prev, {
    ...
    return (
        <div className="container-fluid">
            ...
            <div className="row">
                {
                    musics.map(item => (
                        <MusicItem favoritesMusic={favoritesMusic} item={item} setLikeOrDeslike={setLikeOrDeslike} key={item.id} />
                    ))
                }
            </div>
        </div>
